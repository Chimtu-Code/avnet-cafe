...
const SHARED = `
  * { margin:0; padding:0; box-sizing:border-box; }
  .admin-page { display:flex; flex-direction:column; width:100vw; height:100vh; overflow:hidden; background:#f5f5f5; font-family:'Poppins',sans-serif; }
  ...
  .check-badge { width:22px; height:22px; background:#4caf50; border-radius:4px; display:flex; align-items:center; justify-content:center; color:#fff; font-size:13px; font-weight:700; flex-shrink:0; }
...
const OrdersCompleted = () => {
  ...
  const fetchOrders = async () => {
    const { data, error } = await supabase.from("orders").select("*").eq("status", "completed").order("created_at", { ascending: false });
                ...
                return (
                  <div key={order.id} className="a-card">
                    <div className="card-top">
                      <span>{(order.name || "USER").toUpperCase()} ORDER CONFIRMED</span>
                    ...
                    <div className="bill-block">
                      <div className="bill-head">
                        <p>Bill Details</p>
                        <button className="expand-btn" onClick={() => setExpanded((p) => ({ ...p, [order.id]: !p[order.id] }))}>Item Details ▾</button>
                      </div>
                      {expanded[order.id] && (
                      ...
                      <div className="bill-row"><span>Item Total</span><span>₹{base}</span></div>
                      <div className="bill-row"><span>GST ({GST_LABEL})</span><span>₹{gst}</span></div>
                      <hr className="bill-sep" />
                      <div className="bill-total"><span>TOTAL BILL</span><span>₹{order.total_price}</span></div>
                    </div>
                  </div>
                );
              })
